...
- **`<Card title="…" kicker="…" tone="…" :to="…">`** — карточка контента на подложке блока кода. `tone`: `default`, `accent` (вывод, главное на слайде), `danger` (так нельзя). `to` делает карточку ссылкой на слайд: номер или `routeAlias`.
- **`<Kicker>01 · проблема</Kicker>`** — надзаголовок слайда: моно, мелко, приглушённо. Компонент, а не `###`, потому что первый markdown-заголовок Slidev берёт в title слайда.
- **`<SectionNum>01</SectionNum>`** — крупный номер блока над заголовком в `layout: section`.
- **`<BarChart :items="…" unit="…" />`** — горизонтальный график из одного ряда, `highlight: true` выделяет столбец, о котором говорит слайд.
- **`<Flow :nodes="…" :edges="…" />`** — простая схема потока слева направо, подробнее ниже.
...
### Схемы и графики из данных

Для простых случаев картинка не нужна: данные лежат в слайде, компонент рисует их токенами темы, в обеих темах и с проверенным контрастом.

`<Flow>` принимает колонки по порядку. Колонка — один узел или массив узлов: одиночный рисуется карточкой (`label` + `note`), массив — стопкой плашек (`label` + `tag` справа). Рёбра соединяют соседние колонки: n → 1 сходится шиной, 1 → n расходится, n → m идёт через общую шину. Узел — строка или `{ label, note, tag, tone, to }`, ребро — строка-подпись или `{ label, dashed, tone: 'accent' | 'muted' }`, `null` — ребро по умолчанию.

Данные удобно держать в YAML слайда и передавать целиком:
...
flow:
  nodes:
    - [Prometheus, Loki, Git]
    - { label: ИИ-ассистент, note: MCP client · dry-run, tone: accent }
    - { label: Инженер, note: apply · только человек }
  edges:
    - read
    - { label: propose, dashed: true, tone: muted }
---

<Flow v-bind="$frontmatter.flow" />
```

Короткую схему проще написать в пропсах: `<Flow :nodes="['git push', 'CI', ['staging', 'canary'], 'prod']" :edges="[null, 'deploy', 'promote']" />`.

Геометрия считается из фиксированной высоты плашки, без замеров DOM, поэтому схема одинаково выглядит в dev, экспорте и печати. Плашки однострочные; если схема не влезает по ширине, оберни её в `<Transform :scale="0.8">`. Для сложных графов в Slidev есть встроенный Mermaid.
...
| `--chart-neutral` | «Тихий» столбец `BarChart` (≥ 3:1 к фону и карточке) |
...
Контраст проверяется скриптом `npm run contrast` и падает в CI, если пара опустилась ниже WCAG AA: 4.5:1 для текста, 3:1 для графики (столбцы, маскот).
